...
class TestPresetList:
    ...
    def test_public(self, package):
        assert call_auth("search_preset_preset_list", {"user": ""}, id=package["id"])
    ...
    def test_private(self, package_factory, user, organization_factory):
        ...
        with pytest.raises(tk.NotAuthorized):
            call_auth("search_preset_preset_list", {"user": ""}, id=package["id"])

        assert call_auth("search_preset_preset_list", {"user": user["name"]}, id=package["id"])
